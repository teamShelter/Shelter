...
contract Router is Ownable {
    ...
    uint256 private beforePercentage = 0;
    ...
    function _getNowPrice() private view returns (uint256) {
        return price.div(10 ** 6).mul(_nowPercentage() + beforePercentage) + price;
    }
    ...
    function setPercentage(uint256 _percentage) public onlyOwner returns (bool) {
        beforePercentage = _nowPercentage() + beforePercentage;
    ...
    function getFullPercentage() public view returns (uint256) {
        return _nowPercentage() + beforePercentage;
    }
    
    function getBeforPercentage() public view returns (uint256) {
        return beforePercentage;
    }
    
    function _nowPercentage() private view returns (uint256) {
        uint256 over = now - time;
        return over.mul(percentage).div(31536000);
    }
    ...
    function getNowPercentage() public view returns (uint256) {
        return _nowPercentage();
    }
